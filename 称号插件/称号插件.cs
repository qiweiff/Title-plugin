...
namespace 称号插件
{
    ...
    public class 称号插件 : TerrariaPlugin
    {
        ...
        public void 查称号(CommandArgs args, string? 玩家用户名)
        {
        ...
        private void 聊天(ServerChatEventArgs args)
        {
            if (配置.是否开启称号插件 == true)
            {
                    ...
                    if (TShock.Players[args.Who].Group.Name != TShock.Config.Settings.DefaultGuestGroupName)
                    {
                        if (配置.称号设置.Exists(a => a.玩家用户名 == TShock.Players[args.Who].Name))
                        {
                            ...
                            if (TShock.Players[args.Who].Name == z.玩家用户名)
                            {
                                ...
                                //z.前前缀 + 前缀 + 角色名 + 后缀 + z.后后缀 + "：" + _聊天内容.Text
                                TShock.Utils.Broadcast("", 255, 255, 255);
                                string 聊天格式 = 配置.聊天格式.Replace("{前前缀}", "{0}").Replace("{前缀}", "{1}").Replace("{角色名}", "{2}").Replace("{后缀}", "{3}").Replace("{后后缀}", "{4}").Replace("{聊天内容}", "{5}");
                                string text = String.Format(聊天格式, z.前前缀, 前缀, 角色名, 后缀, z.后后缀, args.Text);
                                if (!args.Handled)
                                {
                                    发送聊天(args, text, z.前前缀, 前缀, 角色名, 后缀, z.后后缀);
                                }
                            }
                        }
                        else
                        {
                            string 聊天格式 = 配置.聊天格式.Replace("{前前缀}", "{0}").Replace("{前缀}", "{1}").Replace("{角色名}", "{2}").Replace("{后缀}", "{3}").Replace("{后后缀}", "{4}").Replace("{聊天内容}", "{5}");
                            string text = String.Format(聊天格式, "", TShock.Players[args.Who].Group.Prefix, TShock.Players[args.Who].Name, TShock.Players[args.Who].Group.Suffix, "", args.Text);
                            if (!args.Handled)
                            {
                                发送聊天(args, text, "", TShock.Players[args.Who].Group.Prefix, TShock.Players[args.Who].Name, TShock.Players[args.Who].Group.Suffix, "");
                            }
                        }
                    }
                }
        ...
        public void 发送聊天(ServerChatEventArgs args, string text, string 前前缀, string 前缀, string 角色名, string 后缀, string 后后缀)
        {
            ...
            TSPlayer.Server.SendMessage(text, TShock.Players[args.Who].Group.R, TShock.Players[args.Who].Group.G, TShock.Players[args.Who].Group.B); ///经过实践，[c/FFFFFF:],[i:]仍然生效
            TShock.Log.Info($"{TShock.Players[args.Who].Name}>>> {text}");
            args.Handled = true;
        }
        public void 聊天检测(ServerChatEventArgs args)
        {
        ...
        public void 聊天气泡(ServerChatEventArgs args, string 前前缀, string 前缀, string 角色名, string 后缀, string 后后缀)
        {
            if (配置.是否开启聊天气泡)
            {
