    ...
    public class 称号插件配置表
    {
        public static void GetConfig()
        {
            try
            {
                if (!File.Exists(path))
                {
                    FileTools.CreateIfNot(Path.Combine(TShock.SavePath, "称号插件配置表.json"), JsonConvert.SerializeObject(称号插件.配置, Formatting.Indented));
                    称号插件.配置 = JsonConvert.DeserializeObject<称号插件配置表>(File.ReadAllText(Path.Combine(TShock.SavePath, "称号插件配置表.json")));
